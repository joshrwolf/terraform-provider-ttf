...
import (
	...
	"github.com/hashicorp/terraform-provider-ttf/internal/envs"
	"github.com/hashicorp/terraform-provider-ttf/internal/types"
)
...
docker run --name 'ttf-k3s-%[1]s' -d -p %[2]d:6443 --privileged -v %[3]s:/etc/rancher/k3s/config.yaml cgr.dev/chainguard/k3s:latest server
docker exec 'ttf-k3s-%[1]s' /bin/sh -c 'tries=0; while ! k3s kubectl wait --for condition=ready nodes --all --timeout 120s && [ $tries -lt 30 ]; do sleep 2; tries=$((tries+1)); done'
docker exec 'ttf-k3s-%[1]s' /bin/sh -c 'tries=0; while ! k3s kubectl wait --for condition=ready pods --all -n kube-system --timeout 120s && [ $tries -lt 30 ]; do sleep 2; tries=$((tries+1)); done'
      `, h.name, port, cfgpath)); err != nil {
			return ctx, fmt.Errorf("creating k3s cluster: %v\n%s", err, out.String())
		}

		kcfg, err := os.CreateTemp("", "ttf-k3s-kubeconfig")
		if err != nil {
			return ctx, err
		}
		defer kcfg.Close()
...
docker exec 'ttf-k3s-%[1]s' /bin/sh -c "cat /etc/rancher/k3s/k3s.yaml" > $temp
...
func (h *K3s) setupConfig() (string, error) {
	f, err := os.CreateTemp("", "ttf-k3s-config")
	if err != nil {
		return "", nil
	}
	defer f.Close()
